<div class="navbar">
    ...
    if($session->__isset('user_name')) { ?>
        <a href="<?php echo BASE_URL ?>?action=logout"><button class="btn logout">Logout</button></a>
        <a href="<?php echo BASE_URL ?>?action=listTickets"><button class="btn list_tickets">List Tickets</button></a>
        <?php
             ...
             if($userPermissionTypeId != USER_PERMISSION_VIEW && $userPermissionTypeId != USER_PERMISSION_UPDATE){
                echo "<a href=".BASE_URL."?action=createTicket><button class='btn create_ticket'>Create Ticket</button></a>";
             }
       } else { ?>
